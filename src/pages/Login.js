...
class Login extends Component {
  ...
  loginWithMetamask() {
    if (window.ethereum) {
      ...
      if (!address) {
        ...
        this.props.fetchMemberDetail(address).then(responseJson => {
          if (responseJson.type === "FETCH_MEMBER_DETAIL_FAILURE") {
              ...
              this.props
                .postEvents(
                  JSON.stringify({
                    id: "",
                    name: "User creation",
                    payload: { address: address, nonce: 0 }
                  })
                )
                .then(resJson => {
            ...
            self.signWithAccessRequest(
              responseJson.items.member.nonce,
  ...
  signWithAccessRequest(nonce, shares) {
    ...
    let self = this;
    let message =
      "Please, sign the following one-time message to authenticate: " + nonce;
    ...
    return new Promise((resolve, reject) =>
      ...
      web3.personal.ecRecover(message, signature, function(error, result) {
        if (!error) {
          localStorage.setItem(
            "loggedUser",
            JSON.stringify({ address: result, nonce })
          );
          self.props.history.push("/");
        } else {
          alert("Error while retrieving your public key.");
