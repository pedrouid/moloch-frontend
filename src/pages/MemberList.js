...
const MemberList = props => (
  <div id="member_list">
    ...
    <Grid className="member_item">
      ...
      <Divider />
      <Grid.Row className="members_row" centered>
        {props.members.map((contributor, idx) => (
          <MemberAvatar {...contributor} key={idx} />
        ))}
      </Grid.Row>
    </Grid>
  </div>
...
class MemberListView extends React.Component {
  ...
  componentDidMount() {
    this.props.fetchMembers().then(responseJson => {
      this.setState({
        totalMembers: this.state.totalMembers + responseJson.items.length
      });
    });
    this.props.fetchConfigFounders().then(responseJson => {
      this.setState({
        totalMembers: this.state.totalMembers + responseJson.items.length
      });
    });
